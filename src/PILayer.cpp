...
std::thread PILayer::startListening() {
	initscr();
	return std::thread([=] {startKeyboardListener();});
}
...
void PILayer::processKey(uint32_t key, double t) {
	...
	if(!longpress){
		...
		case KPX:
			...
			return;
			break;
		case KPH: //sempre vai ter o mesmo comportamento, portanto o código vai aqui.
		{
		...
		case sysState::ON:
		{
			...
			case KBCommands::SYSTEM_TURN_OFF:
				if(lpress)
					gvars.haltSystem = true;
				break;
			case KBCommands::SYSTEM_HELP_COMMANDS:
				//feedback->helpCommands(commands);
				break;
			case KBCommands::SYSTEM_HELP_STOP:
				//feedback->stopHelp();
				break;
